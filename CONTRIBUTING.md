...
Development uses [Bun](https://bun.sh) 1.4 or newer for dependencies, tests, and builds. The published CLI is plain JavaScript and must keep running on Node.js 22+, so do not use `Bun.*` APIs in `src/`; stick to `node:*` modules. CI runs the packed CLI on Node to enforce this.
